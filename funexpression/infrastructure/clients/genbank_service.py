...
class GenBankService:
    ...
    def _ftp_file_exists(self, ftp_url: str, file_name: str):
    ...
    def _create_folder_if_not_exist(self, acession_number:str):
    ...
    def _get_id_by_acession_number(self, acession_number, database = 'assembly'):
    ...
    def _get_summary_by_id(self, organism_id):
    ...
    def _get_full_organims_name(self, summary):
        return summary['DocumentSummarySet']['DocumentSummary'][
                0
            ]['SpeciesName']

    def _get_ftp_url(self, summary):
    ...
    def download_fasta_sequence_by_id(self, acession_number: str):
        try:
            assembly_id = self._get_id_by_acession_number(acession_number, database='assembly')

            record = self._get_summary_by_id(assembly_id)

            full_organism_name = self._get_full_organims_name(record)

            ftp_url = self._get_ftp_url(record)

            fasta_file = ftp_url.split('/')[-1] + '_protein.faa.gz'

            fasta_url = ftp_url + '/' + fasta_file

            if not self._ftp_file_exists(ftp_url=ftp_url, file_name=fasta_file):
                raise ValueError(f'File {fasta_file} not found on the FTP server.')

            temp_genomes_path = self._create_folder_if_not_exist(acession_number)
